...
def user_input():
    ...
    parser.add_argument('-i', '--runs', type=int, default=0, help='Number of runs to execute consecutively')
    ...
    parser.add_argument('-opt', '--opt-algo', type=str, choices=['random', 'bayesian', 'forest', 'gradient'], default='bayesian', help='Algorithm used in optimization process')
...
def main():
    ...
    hsppbo = HSPPBO(problem, logger, args.personal_best, args.personal_previous, args.parent_best,
                    args.alpha, args.beta, args.dynamic_detection_threshold, args.reaction_type, max_iteration_count=400)
